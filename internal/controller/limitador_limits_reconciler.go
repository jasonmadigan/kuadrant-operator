...
func (r *LimitadorLimitsReconciler) buildLimitadorLimits(ctx context.Context, state *sync.Map) ([]limitadorv1alpha1.RateLimit, error) {
	...
	if err := r.processEffectivePolicies(ctx, state, rateLimitIndex); err != nil {
		return nil, err
	}
...
func (r *LimitadorLimitsReconciler) processEffectivePolicies(ctx context.Context, state *sync.Map, rateLimitIndex *ratelimit.Index) error {
	...
	if effectivePolicies, ok := state.Load(StateEffectiveRateLimitPolicies); ok {
		...
		for pathID, effectivePolicy := range effectivePoliciesMap {
			if err := r.processPolicyRules(ctx, pathID, effectivePolicy.Path, effectivePolicy.Spec.Rules(), state, rateLimitIndex); err != nil {
				return err
			}
		}
	}
...
func (r *LimitadorLimitsReconciler) processPolicyRules(ctx context.Context, pathID string, path []machinery.Targetable, rules map[string]kuadrantv1.MergeableRule, state *sync.Map, rateLimitIndex *ratelimit.Index) error {
	...
	limitRules := lo.Filter(lo.Entries(rules),
		func(r lo.Entry[string, kuadrantv1.MergeableRule], _ int) bool {
			return r.Key != kuadrantv1.RulesKeyTopLevelPredicates && r.Key != kuadrantv1alpha1.TokenRulesKeyTopLevelPredicates
		},
	)
	...
	for _, limitRule := range limitRules {
		...
		var policy machinery.Policy
		var found bool

		// try with RateLimitPolicy predicate
		policy, found = lo.Find(kuadrantv1.PoliciesInPath(path, isRateLimitPolicyAcceptedAndNotDeletedFunc(state)), func(p machinery.Policy) bool {
			return p.GetLocator() == mergeableLimit.GetSource()
		})

		// or TokenRateLimitPolicy predicate
		if !found {
			policy, found = lo.Find(kuadrantv1.PoliciesInPath(path, isTokenRateLimitPolicyAcceptedAndNotDeletedFunc(state)), func(p machinery.Policy) bool {
				return p.GetLocator() == mergeableLimit.GetSource()
			})
		}
		...
		case *kuadrantv1alpha1.TokenLimit:
			...
			rateLimits := lo.Map(limit.Rates, func(rate kuadrantv1alpha1.TokenRate, _ int) limitadorv1alpha1.RateLimit {
				maxValue, seconds := rate.ToSeconds()
				return limitadorv1alpha1.RateLimit{
					Namespace:  limitsNamespace,
					MaxValue:   maxValue,
