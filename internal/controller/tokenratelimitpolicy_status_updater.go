...
func (r *TokenRateLimitPolicyStatusUpdater) enforcedCondition(policy *kuadrantv1alpha1.TokenRateLimitPolicy, topology *machinery.Topology, state *sync.Map) *metav1.Condition {
	...
	if len(affectedGateways) == 0 {
		if message != "" {
			return kuadrant.EnforcedCondition(policy, kuadrant.NewErrUnknown(policyKind, fmt.Errorf(message)), false)
		}
		return kuadrant.EnforcedCondition(policy, kuadrant.NewErrUnknown(policyKind, fmt.Errorf("no gateway found for the policy")), false)
	}

	return kuadrant.EnforcedCondition(policy, kuadrant.NewErrUnknown(policyKind, fmt.Errorf(message)), len(overridingPolicies) == 0)
}
