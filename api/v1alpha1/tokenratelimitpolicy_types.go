...
var (
	...
	TokenRulesKeyTopLevelPredicates = "###_TOKEN_TOP_LEVEL_PREDICATES_###"
)
...
func (p *TokenRateLimitPolicy) Rules() map[string]kuadrantv1.MergeableRule {
	...
	if whenPredicates := spec.MergeableWhenPredicates; len(whenPredicates.Predicates) > 0 {
		rules[TokenRulesKeyTopLevelPredicates] = kuadrantv1.NewMergeableRule(&whenPredicates, policyLocator)
	}
...
func (p *TokenRateLimitPolicy) SetRules(rules map[string]kuadrantv1.MergeableRule) {
	...
	for ruleID := range rules {
		if ruleID == TokenRulesKeyTopLevelPredicates {
...
type TokenCounter struct {
	Expression kuadrantv1.Expression `json:"expression"`
}
...
type TokenLimit struct {
	...
	// Counters defines token-based rate limit counters based on CEL expressions which can reference well known selectors
	...
	// +optional
	Counters []TokenCounter `json:"counters,omitempty"`
...
func (l TokenLimit) CountersAsStringList() []string {
	...
	return utils.Map(l.Counters, func(counter TokenCounter) string {
		return string(counter.Expression)
	})
}
